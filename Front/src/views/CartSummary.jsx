...
const CartSummary = () => {
    ...
    const totalPrice = cartItems.reduce((total, product) => total + product.precio * product.cantidad, 0);
            ...
            <div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0">
                <div className="rounded-lg md:w-2/3">
                    {cartItems.map(product => (
                        <div key={product.id} className="justify-between mb-6 rounded-lg bg-white p-6 shadow-md sm:flex sm:justify-start">
                            <img src={`/${product.id}.png`} alt="product-image" className="w-full rounded-lg sm:w-40" />
                            <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                                <div className="mt-5 sm:mt-0">
                                    <h2 className="text-lg font-bold text-gray-900">{product.nombre}</h2>
                                    <p className="mt-1 text-xs text-gray-700">{product.talla}</p>
                                </div>
                                <div className="mt-4 flex justify-between sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
                                    ...
                                    <div className="flex items-center space-x-4">
                                        <p className="text-sm">{product.precio.toFixed(2)} $</p>
                                        <svg 
                ...
                <div className="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3">
                    ...
                    <div className="flex justify-between">
                        ...
                        <p className="text-gray-700">$10000</p>
                    </div>
                    <hr className="my-4" />
                    <div className="flex justify-between">
                        <p className="text-lg font-bold">Total</p>
                        <div>
                            <p className="mb-1 text-lg font-bold">${(totalPrice).toFixed(2)} PES</p>
